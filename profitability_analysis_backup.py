...
value_fields = ("income", "direct_expense", "gross_profit_loss")
...
def calculate_values(accounts, gl_entries_by_account, filters):
        init = {"income": 0.0, "direct_expense": 0.0, "gross_profit_loss": 0.0}
                ...
                for entry in gl_entries_by_account.get(d.name, []):
                        if cstr(entry.is_opening) != "Yes":
                                ...
                                if entry.type == "Expense":
                                        ...
                                        if account_type in ["Direct Expense", "Cost of Goods Sold", "Expenses Included In Valuation"]:
                                                d["direct_expense"] += flt(entry.debit) - flt(entry.credit)

                                # Gross Profit = Revenue - Direct Expenses ONLY
                                d["gross_profit_loss"] = d.get("income") - d.get("direct_expense")

                total_row["income"] += d["income"]
                total_row["direct_expense"] += d["direct_expense"]

        total_row["gross_profit_loss"] = total_row.get("income") - total_row.get("direct_expense")

        # Calculate Gross Profit Margin for total row
        if total_row.get("income") and total_row["income"] != 0:
                total_row["gross_profit_margin"] = flt((total_row["gross_profit_loss"] / total_row["income"]) * 100, 2)
        else:
                total_row["gross_profit_margin"] = 0.0
...
def prepare_data(accounts, filters, total_row, parent_children_map, based_on):
        ...
        for d in accounts:
                ...
                if row.get("income") and row["income"] != 0:
                        row["gross_profit_margin"] = flt((row["gross_profit_loss"] / row["income"]) * 100, 2)
                else:
                        row["gross_profit_margin"] = 0.0
